...
public class StatusList {
    ...
    public static StatusList parse(String jsonString) {
        ...
        try {
            ...
            JSONArray jsonArrayStatuses      = jsonObject.optJSONArray("statuses");
            if (jsonArrayStatuses != null && jsonArrayStatuses.length() > 0) {
                int length = jsonArrayStatuses.length();
                statuses.statusList = new ArrayList<Status>(length);
                for (int ix = 0; ix < length; ix++) {
                    statuses.statusList.add(Status.parse(jsonArrayStatuses.getJSONObject(ix)));
                }
            }
            JSONArray jsonArrayRepost      = jsonObject.optJSONArray("reposts");
            if (jsonArrayRepost != null && jsonArrayRepost.length() > 0) {
                int length = jsonArrayRepost.length();
                statuses.statusList = new ArrayList<Status>(length);
                for (int ix = 0; ix < length; ix++) {
                    statuses.statusList.add(Status.parse(jsonArrayRepost.getJSONObject(ix)));
                }
            }
