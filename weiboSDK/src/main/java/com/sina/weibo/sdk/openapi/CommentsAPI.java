...
public class CommentsAPI extends AbsOpenAPI {
     ...
     * 根据需要来填充该类，可参考legacy包下 {@link com.sina.weibo.sdk.openapi.legacy}
     */
    ...
    public void show(long id, long since_id, long max_id, int count, int page, int authorType, RequestListener listener) {
        WeiboParameters params = buildShowParamsBase(id, since_id, max_id, count, page, authorType);
    ...
    private WeiboParameters buildShowParamsBase(long id, long since_id, long max_id, int count, int page, int authorType) {
        WeiboParameters params = new WeiboParameters(mAppKey);
        params.put("id", id);
        params.put("since_id", since_id);
        params.put("max_id", max_id);
